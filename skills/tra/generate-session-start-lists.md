...
Require applied phase-1 sessions and the canonical applied phase-2 references
TSV. Pass that TSV to the executor with `--references-plan`.
...
2. Read the applied session and reference TSV files. Re-run
   `validate_session_references_plan.py` on the applied reference TSV and stop
   phase 3 if it contains any error.
